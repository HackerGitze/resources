...
knitr::opts_knit$set(root.dir = "/Users/Regitze/OneDrive - Aarhus universitet/Dokumenter/Universitet/Methods 2/resources/classes")
...
```{r making a function}
test_function <- function(number,number2){
  print("results")
...
```{r}
#A for loop in R is used for iterating over a sequence (e.g., a vector or a sequence of numbers) with a predefined number of iterations, whereas a while loop is employed for repeating a block of code as long as a specified condition holds true, allowing for more flexibility in the loop control.
```

### Ex. B: When would you use for-loops and when would you use while-loops? Explain in your own words in 1-2 sentences

```{r}
#for all i's calculate their outputs from a function
#take time for every press on key space until enter is pressed
```
...
number2=0
while (number2 < 5){ # While the number is below 10, do the following
  print(number2)
  number2=number2+0.5
}
```
...
#simulating how many girl are born among 400 births, when probability for girl is 48.8%
n_girls <- rbinom(1, 400, 0.488)
print(n_girls)

#simulating among 1000 births and creating a histogram
n_sims <- 1000
...
#instead of doing it for s in... we can just insert n_sims into the rbiom
...
basketball <- function(shoot_prob,misses_in_a_row,shots_taken){ # starting the function for you

while (misses_in_a_row < 2) {
  shots_taken <- shots_taken + 1
  if (runif(1) > shoot_prob) {
    misses_in_a_row <- misses_in_a_row + 1
  } else {
    misses_in_a_row <- 0  # Reset consecutive misses counter if a shot is made
  }
}
...
for (i in 1:num_simulations) {
  results[i] <- basketball(shoot_prob = 0.6, misses_in_a_row = 0, shots_taken = 0)
}

mean_shots_taken <- mean(results)
sd_shots_taken <- sd(results)
print(paste("Average shots taken to miss two in a row over 1000 simulations:", mean_shots_taken))
print(paste("Standard deviation of shots taken:", sd_shots_taken))
hist(results, main="Histogram of Shots Taken", xlab="Shots Taken", col="lightblue", border="black", breaks=seq(min(results), max(results)+1, by=1))
```
